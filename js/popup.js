...
function humaniser(cle) {
    return cle.replace(/_/g, " ").replace(/^c /, "").replace(/\b\w/g, l => l.toUpperCase());
}

function premierChampValide(props, champs) {
    for (const c of champs) {
        if (props[c] !== undefined && props[c] !== null && props[c] !== "" && props[c] !== "NULL") return props[c];
    }
    return null;
}
...
function nomCarburant(nom) {
    return { "Gazole": "Gazole", "SP95": "SP95", "SP98": "SP98", "E10": "SP95-E10", "E85": "E85", "GPLc": "GPL" }[nom] || nom;
}
...
function construireServices(props) {
    ...
    const icones = {
        "Station de gonflage": "fa-solid fa-wind", "Lavage automatique": "fa-solid fa-spray-can-sparkles",
        "Lavage manuel": "fa-solid fa-soap", "Bornes électriques": "fa-solid fa-charging-station",
        "DAB (Distributeur automatique de billets)": "fa-solid fa-money-bill-wave", "Automate CB 24/24": "fa-solid fa-credit-card",
        "Piste poids lourds": "fa-solid fa-truck", "Location de véhicule": "fa-solid fa-car",
...
function construirePopupCarburant(props) {
    ...
    const lignesPrix = prix.map(c => `
        <div class="popup-carburant-prix ${c.classe}">
            ...
            ${c.maj ? `<div class="popup-carburant-maj-prix">mis à jour le ${echapperHtml(formaterMaj(c.maj))}</div>` : ""}
        </div>
    `).join("");
